...
to refresh-to-display-mode
  ...
  [
    ...
    set-legend-continuous-range maxBiomass 0 18 12 6 true
  ]
  ...
  [
    ...
    [
      carefully
      [ set pcolor 42 + 6 * (1 - mean yield / (maxMeanYield + 1E-6)) ]
      [ set pcolor 49 ]
    ]
    set-legend-continuous-range maxMeanYield 0 18 12 6 true
  ]
...
to setup-plot-crop
  ...
  set-current-plot "Crops biomass"
...
to update-plot-crop
  ...
  [
    cropName ->
    set-current-plot-pen cropName
    ifelse (is-ripe position cropName typesOfCrops) [ plot mean [yield] of patches with [crop = cropName] ] [ plot 0 ]
  ]
