...
to update-biomass [ cropIndex ]
  ...
  set biomass_rate solarRadiation * (item cropIndex RUE) * f_Solar * f_CO2 * f_Temp * (clampMin0 (min (list f_Heat f_Water)))
...
to update-f_Solar [ cropIndex ]
  ...
  let f_Solar_early f_Solar_max / (1 + e ^ (-0.01 * (TT - item cropIndex I_50A)))
...
to refresh-to-display-mode
  ...
  if (display-mode = "crop type")
  [
    ask patches
    [
      set pcolor get-crop-color crop
    ]
    set-legend-crop-type
  ]
  if (display-mode = "biomass (g/m2)")
  [
    let minBiomass min [biomass] of patches
    let maxBiomass max [biomass] of patches

    ask patches
    [
      ifelse (biomass > 0)
      [ set pcolor 52 + 6 * (1 - (biomass - minBiomass) / (maxBiomass + 1E-6 - minBiomass)) ]
      [ set pcolor 59 ]
    ]
    set-legend-continuous-range maxBiomass minBiomass 59 52 7 true
  ]
  if (display-mode = "mean yield (g/m2)")
  [
    let minMeanYield 0
    carefully [ set minMeanYield min [yield] of patches ] [ set minMeanYield 0 ]
    let maxMeanYield 0
    carefully [ set maxMeanYield max [yield] of patches ] [ set maxMeanYield 1E-6 ]

    ask patches
    [
      carefully
      [ set pcolor 42 + 6 * (1 - (mean yield - minMeanYield) / (maxMeanYield + 1E-6 - minMeanYield)) ]
      [ set pcolor 49 ]
    ]
...
to update-plot-crop
  ...
  [
    ...
    plot mean [biomass] of patches with [crop = cropName]
  ]
  ...
  [
    ...
    [
      plotxy (currentyear + (currentDayOfYear / yearLengthInDays)) (mean [yield] of patches with [crop = cropName])
    ]
  ]
...
to setup-yield-performance-data-file
  ...
  let filePath (word "output//yield//SIMPLE-crop-model_withSpatialDiversity_yield-exp_type-of-experiment=" type-of-experiment "_experiment-name=" experiment-name "_initRandomSeed=" experiment-initRandomSeed ".csv")
