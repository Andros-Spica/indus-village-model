...
for (x in 1:nrow(elevation))
{
  for (y in 1:nrow(elevation))
  {
    ...
    {
      flowLinks <- c(flowLinks, c(x, y, downstreamPoint))
    }
  }
}

flowLinks <- matrix(flowLinks, 
                    ncol = 4, 
                    nrow = length(flowLinks) / 4, 
                    byrow = T)
```
...
for (i in 1:nrow(flowLinks))
{
  # notice that x, y, z are interpreted differently than in barplot3d
  segments3d(x = flowLinks[i, c(1, 3)]+0.5,
             z = -1*flowLinks[i, c(2, 4)]-0.5,
             y = c(elevation[flowLinks[i, 1], flowLinks[i, 2]],
                   elevation[flowLinks[i, 3], flowLinks[i, 4]])+0.05,
             color = "blue", lwd = 2)
}
...
iteration: runoff, infiltration, try to send runoff
