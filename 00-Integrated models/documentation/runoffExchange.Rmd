...
The runoff exchange algorithm combines the Soil Water Balance model with the design of two algorithms: (a) one that handles the order of the pairwise exchange between land units and (b) another for calculating the runoff exchanged at every instance.

The algorithm handling the order of runoff exchange (a) follows the same logic that is used to calculate flow_accumulation in the Land model, based on: 
...
The algorithm calculating the runoff effectively moving from one land unit to another (b) is a simplification of the one found on:
...
### Elevation, flow direction and flow accumulation

See extended explanation at [HTML with R walkthrough](https://htmlpreview.github.io/?https://github.com/Andros-Spica/indus-village-model/blob/master/03-land-model/documentation/flowAccumulationOrder.html)
...
### Soil Water Balance with runoff exchange

See extended explanation at [HTML with R walkthrough](https://htmlpreview.github.io/?https://github.com/Andros-Spica/indus-village-model/blob/master/03-land-model/documentation/flowAccumulationOrder.html)

At this point of development (i.e. integrated models), the original soil water balance model is broken down into parts to better connect to the inputs/outputs of other modules and fit the solution for runoff exchange. Note that because of these changes, the original R implementation done by Wallach et al. is no longer used.
...
waterCube <- cube3d(color = surfaceWaterColour, alpha = 0.2)  
  ...
  for (z in 1:dim(elevation)[2])
  {
    if (waterLayers$surfaceWater[x, z] > 0)
    {
      thisWaterCube <- waterCube %>% 
        scale3d(0.5, 0.005 * waterLayers$surfaceWater[x, z], 0.5) %>% 
        translate3d(x + 0.5, elevation[x, z] + 0.005 * waterLayers$surfaceWater[x, z], -1 * z - 0.5)
...
waterCube <- cube3d(color = soilWaterColor, alpha = 0.2)  
...
for (x in 1:dim(elevation)[1])
{
  for (z in 1:dim(elevation)[2])
  {
    ...
    {
      thisWaterCube <- waterCube %>% 
        scale3d(0.5, 0.001 * waterLayers$soilWater[x, z], 0.5) %>% 
        translate3d(x + 0.5, elevation[x, z] - 0.001 * waterLayers$soilWater[x, z], -1 * z - 0.5)
      shade3d(thisWaterCube)
...
```{r}
waterLayers <- solveRunoffExchange(grid = elevation, # elevation is only used as a grid template 
