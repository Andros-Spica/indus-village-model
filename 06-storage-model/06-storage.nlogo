...
[
  ...
  hh_dietUnfullfilled              ; The amount of stock units desired by the household, but currently unsatisfied (stock units).
...
to reset-diet-satisfaction
  ...
  [
    ...
    set hh_dietUnfullfilled hh_dietDesired
  ]
...
to stocks-update
  ...
  ask households with [hh_dietUnfullfilled = 0]
  [
    hh_stocks-sell
  ]
  ...
  ;;; calculate the number of possible transactions as the minimum between supply (market-stocks) and demand (sum [hh_dietUnfullfilled] of households)
  let numberOftransactions min (list (sum market_stocks) (sum [hh_dietUnfullfilled] of households) ) ; (sum [min (list hh_dietUnfullfilled hh_exchangePower)] of households) )

  repeat numberOftransactions
  [
    ask one-of households with [hh_dietUnfullfilled > 0]
    [
      ;;; search & acquire
      hh_stocks-buy
