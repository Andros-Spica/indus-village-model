...
patches-own
[
  ;========= LAND model (imported terrain values) ======================================================================
  ...
  ;;; modified variables
  ;;; These variables are now a list of values per ecological component
  ...
  p_soil_runOffCurveNumber                     ; runoff curve number (0=full retention to 100=full impermeability). list of values per ecological component
  ...
  p_ecol_albedo                     ; albedo or percentage of solar radiation reflected by soil or soil cover. list of values per ecological component

  p_ecol_biomass                    ; total biomass (g/m^2) of ecological communities (vegetation as proxy). list of values per ecological component
]
...
to update-weather
  ...
  ask patches
  [
    set p_netSolarRadiation get-netSolarRadiation solarRadiation
    set p_ETr get-ETr
  ]
...
to-report get-netSolarRadiation [ givenSolarRadiation ]
...
(1 - p_ecol_albedo / 100) * solarRadiation
  let %bareSoil (100 - %wood - %brush - %grass - %water - %crops)

  report

  report map [ i ->  (1 - i / 100) * givenSolarRadiation ] (list
    (get-albedo-of-cover "inland water")
    (get-albedo-of-cover "woodlands")
    (get-albedo-of-cover "shrublands")
    (get-albedo-of-cover "grasslands")
    (
      (get-albedo-of-cover "wet bare not sandy soil") * (%bareSoil / 100) * (1 - %sand / 100) * waterContentRatio +
      (get-albedo-of-cover "dry bare not sandy soil") +
      (get-albedo-of-cover "wet bare sandy soil") +
      (get-albedo-of-cover "dry bare sandy soil")
    )
    (get-albedo-of-cover "croplands")
