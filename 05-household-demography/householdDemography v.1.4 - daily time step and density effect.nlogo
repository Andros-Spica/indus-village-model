...
[
  ...
  carryingCapacity               ; carrying capacity or maximum number of people that can be sustained feed, i.e. mortality is increased in proportion to closeness of population to it (density effect)
...
to set-parameters
  ...
  [
    ...
    set mu-fert maturityAgeInYears + (random 40 - maturityAgeInYears)
    set sigma1-fert max (list 1E-6 (random-float (mu-fert - maturityAgeInYears)))
    set sigma2-fert max (list 1E-6 (random-float ((30 + maturityAgeInYears) - mu-fert)))
...
to-report hh_whichMembersDying
  ...
  [
    memberIndex ->
    (random-float 1 < get-net-mortality-of-day (item memberIndex hh_membersSex) (item memberIndex hh_membersAge))
  ] hh_membersIndexes
...
to-report hh_is-infants-only
  ...
  let maturityAge maturityAgeInYears * yearLengthInDays
...
to-report get-density-effect-of-day
  ...
  let densityEffectOfDay 1 - (1 - densityEffect) ^ (1 / yearLengthInDays)
