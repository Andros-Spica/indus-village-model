compute_density_metrics <- function(
    ...
    mutate(
      ...
      equilibrium_pressure_mean =
        mean(pressure[step >= quantile(step, burn_in_fraction)], na.rm = TRUE),
      # coefficient of variation of pressure during the equilibrium phase
      equilibrium_pressure_variance =
        var(
          pressure[step >= quantile(step, burn_in_fraction)],
          na.rm = TRUE
        ),
      ...
      equilibrium_pressure_to_population_ratio_se =
        equilibrium_pressure_to_population_ratio_cv / sqrt(sum(step >= quantile(step, burn_in_fraction))),
      ...
      time_above_capacity =
        sum(pressure[step >= quantile(step, burn_in_fraction)] > 1) / sum(step >= quantile(step, burn_in_fraction))
    ) |>
    ungroup()
}
